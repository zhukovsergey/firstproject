...
const Comments = ({ comments, setComments }) => {
  ...
  const removeCommentHandler = async (e, id) => {
    e.preventDefault();
    try {
        ...
        >
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-2  ">
              <div>
                <Avatar>
                  <AvatarImage
                    src={`http://localhost:3000` + comment.commented_by.image}
                  />
                  <AvatarFallback>
          ...
          <div>
            {user?._id === comment?.commented_by?._id ||
              (user?.role === "admin" && (
                <CiCircleRemove
                  onClick={(e) => removeCommentHandler(e, comment._id)}
                  className="text-red-500 cursor-pointer text-3xl hover:bg-red-300 hover:text-white rounded-full transition-all duration-300"
                />
              ))}
          </div>
        </div>
